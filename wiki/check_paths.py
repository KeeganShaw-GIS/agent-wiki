...
_WIKI_BANNER = (
    "> **WIKI MANAGED** — CLAUDE.md files in this repo are generated by "
    "[claude-wiki](.claude-wiki/wiki). You can edit them directly, but use "
    "`.claude-wiki/wiki update` to regenerate from source.\n\n"
    "---\n\n"
)
...
def ensure_wiki_doc(rel_path: str, source: str = "manual", quiet: bool = False) -> Path:
    ...
    if not dp.exists():
        ...
        if TEMPLATE_FILE.exists():
            content = TEMPLATE_FILE.read_text().replace("{path}", rel_path or "root")
            dp.write_text(content)
        else:
            label = rel_path if rel_path else "Root"
            dp.write_text(f"# {label}\n\n> Not yet populated. Run `claude-wiki update --scope {label}` to generate.\n")
...
def _verify_symlinks(nodes: list, repo: Path):
    ...
    if untracked:
        print(
            ...
            + "".join(f"    {p or '(root)'}\n" for p in untracked)
            + f"  Run: claude-wiki push  (or update to also generate content)",
            file=sys.stderr,
        )
